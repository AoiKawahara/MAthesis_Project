...
topic_prob <- posterior(lda_5)$topics
df_topic <- df[-342, ] %>%
  mutate(assigned_topics, apply(topic_prob, 1, which.max))
...
```{r sent_topic}
df_topic <- df_cleaned_text[-342, ] %>%
  mutate(assigned_topics, apply(topic_prob, 1, which.max))

df_sent_topic <- pos_neg[-342, ] %>%
  inner_join(df_cleaned_text, by = c("id" = "id")) %>%
  inner_join(df_topic, by = c("text" = "text"))


df_sent_topic$assigned_topics <- assigned_topics


table(duplicated(df_cleaned_text$text))
table(duplicated(df_topic$text))
```
